...
public class ExcelConverter {
    ...
    public void parseOneParamFunctionToExcel(String function, DataCreatorRequestDto request, MathOneParamFunction originalFunction) throws IOException {
        ...
        var range = DoubleStream.iterate(request.rangeStart(), d -> d <= request.rangeEnd()
                        , d -> d + request.step())
                .boxed()
    ...
    public void parseTwoParamFunctionToExcel(String function, DataCreatorRequestDto request, MathTwoParamFunction originalFunction) throws IOException {
        ...
        var range = DoubleStream.iterate(request.rangeStart(), d -> d <= request.rangeStart()
                        , d -> d + request.step())
                .boxed()
                .map(d -> round(d * 100.0) / 100.0)
                .toList();
        ...
        for (Double x : results.keySet().stream().sorted().toList()) {
            ...
            evaluator.evaluateFormulaCell(cellExp);
            Cell cellOriginalExp = row.createCell(2);
            cellOriginalExp.setCellValue(originalFunction.call(x, x));
        }
